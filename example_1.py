...
def camera_calibration(current_path: str) -> tuple:
    ...
    :rtype: tuple
    """
    param_file = join(current_path, "src/camera_params.npz")
    ...
    while True:
        ...
        if ids is not None:
            for i in range(len(ids)):
                marker_id = ids[i][0]
                img_path = join(current_file_path, f"src/photos/monk_{marker_id}.jpg")
        ...
        cv2.imshow("AR Marker Detection: with images", frame)
